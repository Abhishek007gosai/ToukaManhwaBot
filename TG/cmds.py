...
async def start(client, message):
  ...
  await message.reply_photo(
    ...
    caption=(
        ...
        "<b>ʜᴏᴡ ᴛᴏ ᴜsᴇ? Jᴜsᴛ ᴛʏᴘᴇ ᴛʜᴇ ɴᴀᴍᴇ ᴏғ sᴏᴍᴇ ᴍᴀɴɢᴀ ʏᴏᴜ ᴡᴀɴᴛ ᴛᴏ ᴋᴇᴇᴘ ᴜᴘ ᴛᴏ ᴅᴀᴛᴇ.</b>\n"
        "\n"
        "<b>ғᴏʀ ᴇxᴀᴍᴘʟᴇ:</b>\n"
        "<i><code>One Piece</code></i>\n"
        f"<b><blockquote><i>Ping:- {ping}</i></b>\n"
        "<b>ғᴏʀ ᴍᴏʀᴇ ɪɴғᴏʀᴍᴀᴛɪᴏɴ ᴜsᴇ ᴄᴏᴍᴍᴀɴᴅ ʙᴇʟᴏᴡ\n❏ ʙᴏᴛ ᴄᴏᴍᴍᴀɴᴅ\n├/help : ᴄʟɪᴄᴋ ʜᴇʀᴇ</blockquote></b>\n"
...
async def del_handler(_, msg):
  ...
  try:
    ...
    try:
      await retry_on_flood(_.send_message)(
        user_id,
        "<i>Your Premuims Plans End.. Please Buy again or Contact To Owner @EternalsHelplineBot</i>"
      )
    except:
      pass
  except Exception as err:
...
async def help(client, message):
  if Vars.IS_PRIVATE:
    if message.chat.id not in Vars.ADMINS:
      return await message.reply("<code>You cannot use me @EternalsHelplineBot</code>")
